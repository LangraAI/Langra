...
pub fn resize_window_to_popup() {
    println!("[RESIZE] Resizing to popup mode (500x450)");
    if let Some(handle) = APP_HANDLE.get() {
        if let Some(window) = handle.get_webview_window(TRANSLATOR_WIN_NAME) {
            let _ = window.set_size(LogicalSize::new(500.0, 450.0));
            let _ = window.set_min_size(Some(LogicalSize::new(450.0, 400.0)));
        }
    }
}
